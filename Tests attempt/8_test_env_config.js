/*
Test Environment Config
In CI/CD pipelines, tests run against different environments. Write a JavaScript program using a switch statement that takes an environment name stored in a variable and prints the base URL, API key pattern, and timeout. Use const for fixed values and let for the assembled config.

Environments: dev, staging, qa, production/prod. Each has different base URL, API key prefix, timeout, and description.
Input
envName = "staging"
Output
Environment: STAGING Base URL: https://staging-api.testingacademy.com API Key: stg_key_xxxx-xxxx Timeout: 8000ms Description: Staging - Pre-production mirror
*/

function questionName(envName) {
    let answer = "";
    let env = envName.toLowerCase();
    let baseURL = "";
    let timeout;
    let description = "";
    switch (env) {
        case "dev":
            timeout = "6000ms";
            baseURL = envName.toLowerCase();
            description = "Dev - Testing Env";
            break;
        case "qa":
            timeout = "7000ms";
            baseURL = envName.toLowerCase();
            description = "QA - QA Env";
            break;
        case "staging":
            timeout = "8000ms";
            baseURL = env;
            description = "Staging - Pre-production mirror";
            break;
        case "production": case "prod":
            timeout = "9000ms";
            baseURL = envName.toLowerCase();
            description = "PROD - Live env";
            break;
        default:
            timeout = "10000ms";
            baseURL = envName.toLowerCase();
            description = "Unknown env";
            break;
    }
    answer = `Environment: ${envName.toUpperCase()} Base URL: https://${baseURL}-api.testingacademy.com API Key: ${baseURL}_key_xxxx-xxxx Timeout: ${timeout} Description: ${description}`
    return answer;
}
console.log(questionName("prod"));